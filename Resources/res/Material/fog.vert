...
uniform float u_fogEquation; // 0 = linear, 1 = exp, 2 = exp2
...
void main(void)
{
    ...
    vec3 pos = (CC_MVMatrix * a_position).xyz;
    float fogFragCoord = length(pos);                                       //get fog distance
    
    
    if(u_fogEquation == 0.0)
        v_fogFactor = (u_fogEnd-fogFragCoord )/(u_fogEnd-u_fogStart);                //linear fog
    else if(u_fogEquation == 1.0)
        v_fogFactor = exp(-u_fogDensity*fogFragCoord  );                         //exp fog
    else if(u_fogEquation == 2.0)
